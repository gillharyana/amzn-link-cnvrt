...
Example:
`/add_footer_channel AvishkarBots`
...
- **Message @AvishkarBots For More Help -**"""
...
Example: /add_footer_channel AvishkarBots
...
async def forwardUnAuthuser(bot: Client, message: Message):
    ...
    if len(message.command) == 1:
        await message.reply(
            ...
            "\n**Eg:** `/forwardUnAuthuser 2128674349`\n\n",
            quote=True
        )
    else:
        for userID in message.command[1:]:
...
async def addCopyChannelToDB(bot: Client, message: Message):
    ...
    if len(message.command) == 1:
        await message.reply(
            ...
            "Add the bot to the channel and send /id to get the channel ID.\n"
            "\n**Eg:** `/add_forward_channel labelname -10012345678 -> -10087654321` or `/add_forward_channel labelname AvishkarBots -> -10087654321`\n\n",
            quote=True
        )
    else:
        if string_session := db.get_string_session(message.from_user.id):
            ...
            for fchannel in forwardchannels:
                ...
                #     await channel.leave()
                #     await client.stop()
                #     return await message.reply("The Chat ID or Username should of a channel only.\n\nAdd the bot to the channel and send /id to get the channel ID.", quote=True)
...
async def addFooterChannel(bot: Client, message: Message):
    if len(message.command) == 1:
        await message.reply(
            ...
            "\n**Eg:** `/add_footer_channel AvishkarBots`",
            quote=True
        )
    else:
...
🔊 Stay informed with the latest updates by subscribing to our channel: @Amzn_Affiliate_bot""",
            quote=True
        )
    else:
    ...
    if not db.is_user_exist(chat_id):
        ...
        if LOG_CHANNEL:
            ...
            logging.info(
                f"#NewUser :- Name : {message.from_user.first_name} ID : {message.from_user.id}")
    await message.reply(
        f"**Here is my maker's Telegram contact** : @AvishkarBots\n\n"
        f"• __Report a bug...__\n"
